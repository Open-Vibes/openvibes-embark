...
async function render(repo: string, repoUrl: string, file: string): Promise<{ html: string; headings: Heading[] }> {
  ...
  renderer.heading = function ({ tokens, depth }) {
    // The heading's inline markup has to be PARSED, not taken from `raw`: half
    // of this project's headings are `\`start\`` or `\`member connect\``, and the
    // raw text renders them as literal backticks in the h2 and again in the
    // table of contents. The plain-text form (tags stripped) is what the TOC
    // and the slug want; the parsed HTML is what the heading itself wants.
...
async function run(): Promise<void> {
  ...
  // The real version, read from the project itself. The site used to show the
  // word "latest" where a version belongs, which tells a visitor nothing about
  // whether the thing they installed is current.
  ...
  // A SEPARATE module, deliberately: the landing page shows the version, and
  // importing it from docs.ts would pull the entire rendered documentation
  // (hundreds of kB) into the landing bundle for the sake of one string.
  await writeFile(
    join(out, "version.ts"),
    `// GENERATED by scripts/sync-docs.ts — do not edit by hand.\n` +
      `/** The agentistics version this site was built against. */\n` +
      `export const AGENTISTICS_VERSION = ${JSON.stringify(version)}\n`,
  );

  console.log(`\nwrote src/generated/docs.ts (${Object.keys(docs).length} pages) + version.ts`);
}

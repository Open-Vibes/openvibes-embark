...
const T: Record<Lang, Record<string, string>> = {
  en: {
    ...
    "hero.sub.3": "parsed straight from six coding agents.",
    ...
    "feat.5.desc": "Token and cost distribution across every model in your usage history, from any harness. Filter by project to identify which workstreams lean most heavily on expensive models. Donut chart and per-model table with share percentages.",
    "feat.6.title": "100% local — zero cloud, zero telemetry",
    "feat.6.desc": "Reads directly from each harness's local session files — ~/.claude/, ~/.codex/, ~/.gemini/ and more. No cloud sync, no account creation, no analytics, no telemetry. A single binary that parses everything locally and serves it from your machine.",
    ...
    "how.step2.desc": "Starts the api + MCP on port 47291 and the web dashboard on port 47292. Every harness's local session files are read directly — no config needed.",
  ...
  pt: {
    ...
    "hero.sub.3": "direto de seis agentes de código.",
    ...
    "feat.5.desc": "Distribuição de tokens e custos por todos os modelos do seu histórico, de qualquer harness. Filtre por projeto para identificar quais fluxos de trabalho usam mais os modelos mais caros. Gráfico donut e tabela por modelo.",
    "feat.6.title": "100% local — sem nuvem, sem telemetria",
    "feat.6.desc": "Lê diretamente dos arquivos de sessão locais de cada harness — ~/.claude/, ~/.codex/, ~/.gemini/ e mais. Sem sync na nuvem, sem criação de conta, sem analytics, sem telemetria. Um binário único que parseia tudo localmente.",
    ...
    "how.step2.desc": "Inicia a api + MCP na porta 47291 e o dashboard web na porta 47292. Os arquivos de sessão locais de cada harness são lidos diretamente — sem configuração.",

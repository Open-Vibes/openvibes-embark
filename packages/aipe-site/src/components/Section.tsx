...
interface SectionProps {
  ...
  narrow?: boolean;
  /**
   * Header alignment. Left is the page default; a few sections centre their
   * header to vary the scroll rhythm without leaving the visual system.
   */
  align?: "left" | "center";
}
        ...
        {(eyebrow || title || lead) && (
          <Reveal>
            <header className={`mb-12 sm:mb-16 ${centered ? "text-center" : ""}`}>
              ...
              {title && (
                <h2
                  className={`font-display text-3xl sm:text-4xl font-semibold text-text leading-tight ${centered ? "mx-auto max-w-3xl" : "max-w-2xl"}`}
                >
                  {title}
                </h2>
              )}
              {lead && (
                <p
                  className={`mt-4 text-base sm:text-lg text-muted leading-relaxed ${centered ? "mx-auto max-w-2xl" : "max-w-2xl"}`}
                >
                  {lead}
                </p>
              )}

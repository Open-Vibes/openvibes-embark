...
export default function DocsSidebar({ activeSlug, onNavigate }: DocsSidebarProps) {
  ...
  return (
    <nav className="w-56 shrink-0 text-sm" aria-label="Documentation">
      {groups.map(({ group, pages }) => (
        <div key={group} className="mb-6">
          <p className="font-mono text-[11px] uppercase tracking-wider text-faint mb-2">{GROUP_LABEL[group]}</p>
